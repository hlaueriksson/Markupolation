...
- Two sharp edges encoding creates, both covered by `EncodingTests`: markup assembled into a `string` before it reaches an element is encoded whole, and a conditional mixing elements and text collapses to `string` (because `Element` converts to `string`, not the reverse) and is then encoded. Cast one branch to `Content`.
