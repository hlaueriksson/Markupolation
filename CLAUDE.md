...
- The `Content(string?)` constructor, `Content.Raw`, `Element(string)` and `Attribute(string, string?)` are **raw** — they exist so the library and its users can wrap markup verbatim. `Content.Raw` is the only opt-out, because encoding is the default.
